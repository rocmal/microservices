...
// Utility: add realistic 5-10 minute gaps
...
// Sleep helper for realtime loop
function wait(ms) {
...
async function insertBatch() {
    const now = new Date();
    const batchSize = 20; // how many orders every 5 minutes

    console.log(`\n⏱ Inserting batch at ${now.toLocaleString()}...`);

    for (let i = 0; i < batchSize; i++) {
        const orderId = faker.number.int({ min: 10000, max: 99999 });

        // Created within last 5 minutes window
        const created_date = new Date(now.getTime() - faker.number.int({ min: 1, max: 300 }) * 1000);
